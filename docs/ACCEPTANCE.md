...
5. 确认 Codex 未安装时明确提示微软联网限制，不显示“完整离线安装”。
6. DeepSeek 路径中确认 Key 输入不回显，CC Switch 显示导入确认。
7. OCR 使用测试图片完成一次真实识别；原生多模态提示可跳过。
8. 北大法宝完成一次最小检索调用。
9. 重启 Codex，确认已选插件和 Skills 可见。
10. 检查 `%LOCALAPPDATA%\CodexEasyStart\logs`，确认无 API Key。
